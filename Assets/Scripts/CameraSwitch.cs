...
{
    [Header("시네머신 설정")]
    public CinemachineVirtualCamera targetCam; // Player(1) 전용 가상 카메라

    [Header("대상 스크립트 연결")]
    public PlayerMovement originalPlayer;  // 진짜 플레이어의 이동 스크립트
    public PlayerMovement secondaryPlayer; // 복사된 Player(1)의 이동 스크립트

    private bool isPlayerInside = false; // 플레이어가 범위 안에 있는지 여부

    void Start()
    {
        // 1. Player(1) 카메라의 우선순위를 낮춰서 시작 (Player 카메라가 보통 10임)
    ...
    // 트리거 체크: "Player" 태그를 가진 물체가 들어오면 조작 가능 상태로 변경
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player"))
        {
            isPlayerInside = true;
            Debug.Log("전환 범위 진입: 클릭하면 캐릭터를 바꿀 수 있습니다.");
        }
    }

    // 트리거 체크: 범위를 벗어나면 조작 불가능 상태로 변경
